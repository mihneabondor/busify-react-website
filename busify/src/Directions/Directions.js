...
function Directions() {
    ...
    const getDestinationAddressFromParam = async () => {
        try {
            const url = `https://nominatim.openstreetmap.org/reverse?lat=${searchParams.get("destLat")}&lon=${searchParams.get("destLng")}&format=json`;
            const data = await fetch(url, { headers: { "User-Agent": "Busify" } });
            const resp = await data.json();
            const address = resp.address;
            setDestinationSearchValue(`${searchParams.get("destName")}, ${address.road || ""} ${address.house_number || ""}`);
            setDestinationCoords({ lat: parseFloat(searchParams.get("destLat")), lon: parseFloat(searchParams.get("destLng")) });
        } catch (e) {
            console.log(e);
        }
    };
    ...
    const getSuggestions = async (input) => {
        try {
            const url = `https://photon.komoot.io/api/?q=${input}&lat=46.7712&lon=23.6236&limit=3`;
    ...
    useEffect(() => {
        ...
        getUserAddress();
        getDestinationAddressFromParam();
    }, []);
