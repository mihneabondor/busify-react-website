...
function Traseu() {
    ...
    const addMarker = (stop, index) => {
        ...
        if(stop === selectedStop)
            el.className += ' traseu-marker-selected'
    ...
    const addPolyline = useCallback(async (routeId) => {
        // if (!map.current.getSource('route')) {
            try {
                var url = 'https://busifyserver.onrender.com/shapes?shapeid=' + routeId;
    ...
    useEffect(() => {
        if (selectedStop && stopElementsRef.current[selectedStop.stop_id]) {
            const element = stopElementsRef.current[selectedStop.stop_id];
            const container = scrollContainerRef.current;

            if (element && container) {
                // Scroll smoothly to the selected stop
                            ...
                            >
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        zIndex: 1,
                                        background: selectedStop === step ? '#915FA8' : '#F8F9FA',
                                        color: selectedStop === step ? 'white' : 'black',
                                    }}
                                >
                                    {index + 1}
                                </div>
                                ...
                                <div className="mt-2" style={{
                                    ...
                                    wordWrap: "break-word",
                                    whiteSpace: "normal",
                                    color: selectedStop === step ? '#915FA8' : 'gray'
                                }}>
                                    {step.stop_name}
                                </div>
                            </div>
